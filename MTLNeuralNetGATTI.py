...
def forward_propagation(X, parameters, layers_list):
    ...
    #print('BEFORE standard NN this is X', X)
    #print('BEFORE standard NN this is parameters', parameters)
    L0 = 1
...
def compute_cost(X, deltas, deltas_hat, area, scaling_factors, labelsIdx):
    
    LES_Cp = tf.gather(X, labelsIdx, axis=0)
    
    rescaled_deltas    = tf.add(tf.multiply(deltas,scaling_factors[3]),scaling_factors[2])
    rescaled_NN_deltas = tf.add(tf.multiply(deltas_hat,scaling_factors[3]),scaling_factors[2])
    
    rescaled_LES_Cp  = tf.add(tf.multiply(LES_Cp,scaling_factors[1][labelsIdx]),scaling_factors[0][labelsIdx])
    rescaled_HF_Cp   = tf.add(rescaled_deltas, rescaled_LES_Cp)
    rescaled_NN_Cp   = tf.add(rescaled_NN_deltas, rescaled_LES_Cp)
    NN_RMSE = compute_RMSE(rescaled_HF_Cp, rescaled_NN_Cp, area)
    LF_RMSE = compute_RMSE(rescaled_HF_Cp, rescaled_LES_Cp, area)
...
def model(X_train, Y_train, X_dev, Y_dev, X_test, Y_test, layers_list, areaIdx,
    ...
    for epoch in range(num_epochs):
        ...
        for (minibatch_X, minibatch_Y) in train_minibatches:
            ...
            with tf.GradientTape() as tape:
                ...
                minibatch_cost = tf.divide(NN_RMSE,1)
    ...
    for (train_minibatch_X, train_minibatch_Y) in train_minibatches:
        ...
        train_cost = tf.divide(NN_RMSE,1)
